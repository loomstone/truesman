...
async def tool_get_business_hours(request: Request) -> dict:
    ...
    if not hours.data:
        return {"result": "I don't have hours information available right now."}
...
async def tool_check_availability(request: Request) -> dict:
    ...
    rules_result = sb.table("project_reservation_rules").select("*").eq(
        "project_id", project["id"]
    ).single().execute()
    ...
    h = hours_result.data
    open_time = datetime.strptime(h["open_time"][:5], "%H:%M")
    close_time = datetime.strptime(h["close_time"][:5], "%H:%M")
